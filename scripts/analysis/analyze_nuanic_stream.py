...
print(
    f"   Total duration: {(df['timestamp'].iloc[-1] - df['timestamp'].iloc[0]).total_seconds():.2f} seconds"
)
print()
...
print(
    f"   Raw stress range: {df['stress_raw'].min()} - {df['stress_raw'].max()} (0-255 scale)"
)
print(
    f"   Stress % range: {df['stress_percent'].min():.1f}% - {df['stress_percent'].max():.1f}%"
)
...
df["stress_category"] = pd.cut(
    df["stress_percent"], bins=stress_bins, labels=stress_labels, include_lowest=True
)
...
print(
    f"   Full packet hex length: {df['full_packet_hex'].str.len().mean():.0f} chars (avg)"
)
