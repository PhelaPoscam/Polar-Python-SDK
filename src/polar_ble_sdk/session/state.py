...
The dashboard state holds only what a panel renders. Everything rate-related —
live Hz, session totals, first/last timestamps — lives in the
:class:`~polar_ble_sdk.metrics.rate_tracker.RateTracker` the feeds write to.
"""
...
def feed_ppg(
    data: Any, state: dict[str, Any], tracker: RateTracker, key: str = "ppg"
) -> None:
    """Update state and rate tracker from optical PPG callback data."""
...
def feed_acc(
    data: Any, state: dict[str, Any], tracker: RateTracker, key: str = "acc"
) -> None:
    """Update state and rate tracker from 3D Accelerometer callback data."""
    _timestamp, samples = data
...
def feed_gyro(
    data: Any, state: dict[str, Any], tracker: RateTracker, key: str = "gyro"
) -> None:
    """Update state and rate tracker from 3D Gyroscope callback data."""
    _timestamp, samples = data
...
def feed_mag(
    data: Any, state: dict[str, Any], tracker: RateTracker, key: str = "mag"
) -> None:
    """Update state and rate tracker from 3D Magnetometer callback data."""
    _timestamp, samples = data
...
def feed_ecg(
    data: Any, state: dict[str, Any], tracker: RateTracker, key: str = "ecg"
) -> None:
    """Update state and rate tracker from ECG callback data."""
    _timestamp, samples = data
...
def feed_ppi(
    data: Any, state: dict[str, Any], tracker: RateTracker, key: str = "ppi"
) -> None:
    """Update state and rate tracker from Peak-to-Peak Interval (PPI) callback data."""
    if not data:
        return
    state["ppi_count"] += len(data)
    tracker.track(key, len(data))
    last = data[-1]
    if len(last) >= 4:
...
def make_callback(
    ...
    ``key`` names the stream inside the tracker; dual sessions pass
    ``"h10_acc"``/``"sense_acc"`` so one tracker can hold both devices.
    """
    fn = _FEEDERS[kind]
    tracker_key = key or kind

    def cb(data: Any) -> None:
        fn(data, state, tracker, tracker_key)
...
def unwrap_vector(state: dict[str, Any], raw_key: str) -> tuple[Any, Any, Any]:
